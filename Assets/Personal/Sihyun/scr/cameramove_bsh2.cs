...
public class cameramove_bsh2 : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            GetComponent<Camera>().orthographicSize += ((dum-4)- GetComponent<Camera>().orthographicSize) / 10;
            transform.position = new Vector3(transform.position.x - (transform.position.x - pl.transform.position.x) / 10, transform.position.y - (transform.position.y - pl.transform.position.y) / 10, -10);
        }
    }
}
